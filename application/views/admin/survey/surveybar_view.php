<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    var display_on_mouseover = function() {
      var $target = $(this);
      $('span',$target).addClass('hovered').removeClass('unhovered');
    }

    var hide_on_mouseout = function() {
      var $target = $(this);
      $('span',$target).removeClass('hovered').addClass('unhovered');
    }

    var toggle_expand = function(triggered) {
      var $target = $(this);
      $target.toggleClass('fa-caret-square-o-left fa-caret-square-o-right');
      $("#sidebar").toggleClass('unexpanded span2 span1');
      $("#main-content").toggleClass('span11 span10');
      $("#sidebar i").toggleClass('fa-2x');

      if(triggered !== undefined && triggered.type === "click") {
        expanded = !expanded;
        $.cookie('unexpanded', expanded);
    ...
    $("#main-row").on('mouseover', '#sidebar.unexpanded a', display_on_mouseover)
                  .on('mouseout', '#sidebar.unexpanded a', hide_on_mouseout);
    
    expanded = $.cookie('unexpanded') === "true";
    if(!expanded) {
      $('.expandable').toggleClass('fa-caret-square-o-left fa-caret-square-o-right');
      toggle_expand();
    }
  });
</script>
<div class="row-fluid" id="main-row">
  <div id="sidebar" class="span2">
    ...
    <div class="panel">
      <dl>
        <dt>
          <a href="<?php echo $this->createUrl("admin/survey/sa/view/surveyid/$surveyid") ?>" id="survey-title">
    ...
    <div class="well">
      <ul class="nav nav-list">
          ...
          <li>
            <?php if($activated): ?>
              <a href="<?php echo $this->createUrl("admin/survey/sa/deactivate/surveyid/$surveyid"); ?>">
                <i class="fa fa-exclamation red fa-fw"></i> 
                <span><?php $clang->eT("Stop this survey") ?></span>
              </a>
            <?php else: ?>
        ...
        <?php else: ?>
          <li>
            <a href="#" class="red">
              <span><?php $clang->eT("Survey cannot be activated. Either you have no permission or there are no questions.") ?></span>
            </a>
          </li>
        <?php endif ?>
